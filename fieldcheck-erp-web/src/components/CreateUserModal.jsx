...
export default function CreateUserModal({ currentRole, loading, onClose, onSubmit }) {
  ...
  const canCreateManagers = currentRole === 'administrador';
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="modal create-user-modal" onMouseDown={(event) => event.stopPropagation()}>
        <header className="modal-header"><div><h2>Novo usuário</h2><p>Crie o login e defina o acesso à empresa.</p></div><button className="icon-button" type="button" onClick={onClose} title="Fechar"><X size={20} /></button></header>
        <form className="create-user-form" onSubmit={submit}>
          ...
          <label>Perfil<select value={form.papel} onChange={(event) => field('papel', event.target.value)}><option value="tecnico">Técnico</option>{canCreateManagers ? <><option value="supervisor">Supervisor</option><option value="administrador">Administrador</option></> : null}</select></label>
          <label>Senha provisória<div className="password-field"><input type={showPassword ? 'text' : 'password'} value={form.senha} onChange={(event) => field('senha', event.target.value)} autoComplete="new-password" minLength={8} required /><button type="button" className="field-icon" onClick={() => setShowPassword((value) => !value)} title={showPassword ? 'Ocultar senha' : 'Mostrar senha'}>{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button><button type="button" className="field-icon" onClick={copiarSenha} title="Copiar senha"><Copy size={18} /></button></div><small className={passwordStrong ? 'valid' : ''}>Use pelo menos 8 caracteres.</small></label>
          ...
          <footer className="form-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancelar</button><button type="submit" className="primary-button" disabled={loading}><Save size={18} /> {loading ? 'Criando...' : 'Criar usuário'}</button></footer>
        </form>
      </section>
    </div>
  );
