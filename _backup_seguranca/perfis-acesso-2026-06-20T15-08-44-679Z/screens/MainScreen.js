...
export default function MainScreen({ navigation, usuarioLogado, acessoComercial, onLogout, resumoHome = {} }) {
  ...
  const acoesRapidas = gestor
    ? acoesRapidasBase
    : acoesRapidasBase.filter((item) => item.rota !== 'Ordens');
  ...
  const atalhos = gestor
    ? atalhosBase
    : atalhosBase.filter((item) => !['Clientes', 'Modelos', 'Assinatura'].includes(item.rota));
