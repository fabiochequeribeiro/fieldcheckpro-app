    ...
    Alert.alert('Excluir modelo', 'Deseja remover este modelo de checklist?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            await supabase.from('modelos_checklist_genericos_itens').delete().eq('modelo_id', id).eq('empresa', EMPRESA_ATUAL);
            await supabase.from('modelos_checklist_genericos').delete().eq('id', id).eq('empresa', EMPRESA_ATUAL);
          } catch (erro) {
