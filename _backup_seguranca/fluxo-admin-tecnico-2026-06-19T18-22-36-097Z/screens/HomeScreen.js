...
async function buscarTecnicoAtivoPorEmail(email) {
  ...
  const empresaAtual = String(EMPRESA_ATUAL || '').trim().toLowerCase();
...
async function buscarPedidoLocal(numeroPedido) {
  ...
  return lerJsonStorage(`${STORAGE_PEDIDO_DETALHE_CACHE}_${numero}`, null);
}
    ...
    try {
      ...
      if (erroInserirVisita) {
        ...
        const { error: erroAtualizarCompleto } = await supabase
          .from('visitas')
          .update({ ...dadosCompletos, empresa: EMPRESA_ATUAL })
    ...
    try {
      ...
      if (data?.session?.user) {
        ...
        setUsuario({
          ...
          empresa: tecnico?.empresa || EMPRESA_ATUAL,
          tecnico,
        });
      }
    } catch (erro) {
