...
export default function HomeScreen({ route, usuarioLogado }) {
  ...
  function Campo({ label, value, onChangeText, editable = true }) {
    return (
      <View style={styles.campoBox}>
        <Text style={styles.label}>{label}</Text>
        <TextInput style={[styles.input, !editable ? { backgroundColor: '#f1f5f9', color: '#475467' } : null]} value={value} onChangeText={onChangeText} placeholder={label} editable={editable} selectTextOnFocus={editable} />
      </View>
    );
  }
