...
import { obterEmpresaAtual, obterTecnicoAtualId, obterUsuarioAtualId, usuarioPodeGerenciar } from '../utils/sessaoOperacional';
...
function normalizarFotoOcorrencia(foto) {
  if (!foto) return null;
  if (typeof foto === 'string') return { uri: foto };
  if (typeof foto === 'object') return { ...foto };
  return null;
}

function obterUriPreviewFoto(foto) {
  const valor = typeof foto === 'string'
    ? foto
    : foto?.uri || foto?.dataUri || foto?.uriOriginal || foto?.signed_url || foto?.public_url || '';
  if (typeof valor !== 'string') return '';
  const inicio = valor.toLowerCase();
  return inicio.startsWith('file:') || inicio.startsWith('content:') || inicio.startsWith('data:image/') || inicio.startsWith('http:') || inicio.startsWith('https:')
    ? valor
    : '';
}

async function prepararFotosParaEdicao(fotos = []) {
  const lista = (Array.isArray(fotos) ? fotos : [])
    .map(normalizarFotoOcorrencia)
    .filter(Boolean);

  return Promise.all(lista.map(async (foto) => {
    if (obterUriPreviewFoto(foto) || !foto.storage_path) return foto;
    try {
      const { data, error } = await supabase.storage
        .from('fieldcheck-media')
        .createSignedUrl(foto.storage_path, 3600);
      if (!error && data?.signedUrl) return { ...foto, signed_url: data.signedUrl };
    } catch (erro) {
      console.log('Nao foi possivel preparar a foto da ocorrencia:', erro);
    }
    return foto;
  }));
}
...
function normalizarOcorrencia(item = {}) {
  ...
  return {
    ...
    fotos: (Array.isArray(item.fotos) ? item.fotos : []).map(normalizarFotoOcorrencia).filter(Boolean),
...
export default function OcorrenciasScreen() {
  ...
  async function abrirEdicao(item) {
    try {
      const ocorrencia = normalizarOcorrencia(item);
      const fotos = await prepararFotosParaEdicao(ocorrencia.fotos);
      setEditandoId(ocorrencia.id);
      setForm({ ...ocorrencia, fotos });
      setModalAberto(true);
    } catch (erro) {
      console.log('Erro abrindo edicao da ocorrencia:', erro);
      Alert.alert('Nao foi possivel editar', 'Os dados da ocorrencia nao puderam ser preparados. Atualize a tela e tente novamente.');
    }
  }
  ...
  async function sincronizarOcorrencia(registro) {
    try {
      ...
      const payload = {
        ...
        tecnico_id: registro.tecnico_id || obterTecnicoAtualId(),
        criado_por: registro.criado_por || obterUsuarioAtualId(),
      };
  ...
  async function gerarPdf(item, acao = 'compartilhar') {
    try {
      ...
      if (acao === 'email') {
        ...
        if (item.status === 'aberta') await atualizarStatus(item, 'enviada para matriz');
        return;
      }
  ...
  async function apagarOcorrencia(item) {
    if (!usuarioPodeGerenciar()) {
      Alert.alert('Ação restrita', 'Ocorrências enviadas não podem ser apagadas pelo técnico.');
      return;
    }
    Alert.alert(
      'Apagar ocorrência',
            ...
            ocorrenciasFiltradas.map((item) => (
              <View key={item.id} style={styles.card}>
                ...
                <View style={styles.acoesLinha}>
                  {(usuarioPodeGerenciar() || String(item.status || 'aberta').toLowerCase() === 'aberta') ? <TouchableOpacity style={styles.botaoAcao} onPress={() => abrirEdicao(item)}>
                ...
                {usuarioPodeGerenciar() ? <View style={styles.statusLinha}>
                  {STATUS.map((status) => (
                    <TouchableOpacity
                      key={status}
                ...
                {usuarioPodeGerenciar() ? <TouchableOpacity style={styles.botaoApagar} onPress={() => apagarOcorrencia(item)}>
              ...
              <Campo label="Fotos">
                ...
                <View style={styles.fotosGrid}>
                  {(form.fotos || []).map((foto, index) => {
                    const uriPreview = obterUriPreviewFoto(foto);
                    const chave = foto?.storage_path || uriPreview || `foto-${index}`;
                    return (
                      <View key={chave} style={styles.fotoBox}>
                        {uriPreview ? (
                          <Image source={{ uri: uriPreview }} style={styles.fotoPreview} />
                        ) : (
                          <View style={[styles.fotoPreview, styles.fotoIndisponivel]}>
                            <Ionicons name="image-outline" size={30} color="#64748b" />
                            <Text style={styles.fotoIndisponivelTexto}>Foto salva. Previa indisponivel.</Text>
                          </View>
                        )}
...
const styles = StyleSheet.create({
  ...
  fotoIndisponivel: { alignItems: 'center', justifyContent: 'center', padding: 16 },
  fotoIndisponivelTexto: { color: '#64748b', fontWeight: '700', marginTop: 8, textAlign: 'center' },
