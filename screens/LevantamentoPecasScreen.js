...
export default function LevantamentoPecasScreen() {
    ...
    try {
      ...
      return levantamentos.map((levantamento) => {
        ...
        return {
          ...
          tecnico_id: levantamento.tecnico_id || null,
          criado_por: levantamento.criado_por || null,
  ...
  async function salvarLevantamentoSupabase(registro, pecasValidas) {
    try {
      ...
      const cabecalho = {
        ...
        status: ['finalizado', 'enviado', 'aprovado'].includes(String(registro.status || 'Finalizado').toLowerCase()) ? 'Rascunho' : (registro.status || 'Rascunho'),
        atualizado_em: agora,
      ...
      const statusFinal = registro.status || 'Finalizado';
      if (statusFinal !== cabecalho.status) {
        const { error: erroFinalizar } = await supabase
          .from('levantamentos_pecas_obra')
          .update({ status: statusFinal, atualizado_em: agora })
          .eq('id', registro.id)
          .eq('empresa', obterEmpresaAtual());
  ...
  async function salvarGarantiaNoCelular(listaPecas = pecas) {
    try {
      ...
      const registro = {
        ...
        obra,
        tecnico,
        tecnico_id: existente?.tecnico_id || obterTecnicoAtualId(),
        criado_por: existente?.criado_por || obterUsuarioAtualId(),
        observacoesGerais,
  ...
  function calcularResumo(listaPecas = []) {
    const pecasPreenchidas = listaPecas.filter(
      (peca) => String(peca.codigo || '').trim() || String(peca.descricao || '').trim()
    );
    return STATUS_PECAS.map((status) => ({
      ...status,
      total: pecasPreenchidas.filter((peca) => peca.status === status.id).length,
    }));
  }
  ...
  function abrirLevantamento(item) {
    if (!usuarioPodeGerenciar() && item.sincronizacao === 'sincronizado' && String(item.status || '').toLowerCase() === 'finalizado') {
      Alert.alert('Levantamento finalizado', 'Este registro já foi enviado e está disponível somente para consulta, PDF e Excel.');
      return;
    }
  ...
  function excluirLevantamento(id) {
    if (!usuarioPodeGerenciar()) {
      Alert.alert('Ação restrita', 'Somente um gestor pode excluir levantamentos.');
      return;
    }
    Alert.alert('Excluir histórico', 'Deseja remover este levantamento do histórico?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          await salvarHistorico(novaLista);
          try {
            await supabase.from('levantamentos_pecas_obra_itens').delete().eq('levantamento_id', id).eq('empresa', obterEmpresaAtual());
            await supabase.from('levantamentos_pecas_obra').delete().eq('id', id).eq('empresa', obterEmpresaAtual());
          } catch (erro) {
                ...
                return (
                  <View key={item.id} style={styles.historicoCard}>
                    ...
                    <View style={styles.historicoAcoes}>
                      ...
                      {usuarioPodeGerenciar() ? <TouchableOpacity style={styles.historicoBotaoExcluir} onPress={() => excluirLevantamento(item.id)}>
